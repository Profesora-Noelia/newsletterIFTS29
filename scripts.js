            ...
            if (contenedor) {
                contenedor.innerHTML = `
                    <p style="
                        ...
                        border-left: 4px solid #153A4C;
                    ">
                        [Área de desarrollo] El texto se cargará aquí cuando el archivo 
                        <strong>${rutaArchivo}</strong> sea creado.
                    </p>
                `;
            }
        });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 01. Tecnicatura Superior en Desarrollo de Software de Simuladores
    cargarTexto(
        'texto-simuladores',
        'textos/texto-simuladores.html'
    );

    // 02. Congreso
    cargarTexto(
        'texto-congreso',
        'textos/congreso.html'
    );

    // 03. Metodologías activas
    cargarTexto(
        'texto-metodologias',
        'textos/metodologias.html'
    );
    ...
    // 05. Familiarización
    cargarTexto(
        'texto-familiarizacion',
        'textos/familiarizacion.html'
    );

    // 06. Próximamente
    cargarTexto(
        'texto-loqueviene',
        'textos/loqueviene.html'
    );
});


// 3. FUNCIÓN PARA NAVEGAR POR LAS PESTAÑAS (TABS)
...
function openTab(evt, tabName) {
    ...
    // y añadir la clase "active" al botón presionado
